...
class LinearProbeClassifier(nn.Module):
    def __init__(self, cfg, clip_model, base_text_features):
        ...
        if self.initialization == "RANDOM":  # RI - LP
        ...
        elif "ZS" in self.initialization:  # ZS - LP
            print("Using Zero-Shot initialization in Linear Probing", end="\n")
            self.prototypes = nn.Parameter(base_text_features.clone())
        elif "PSEUDORANDOM" in self.initialization:  # ZS - LP
            print("Using PSEUDORANDOM initialization in Linear Probing", end="\n")
            scales = base_text_features.clone().float().sum(0)
            rand_prot = torch.nn.init.kaiming_normal_(torch.empty(base_text_features.shape))
            scales_random = rand_prot.clone().float().sum(0)
            initProt = (rand_prot * (scales/scales_random)).detach().clone()
            self.prototypes = nn.Parameter(initProt)
        elif "TR" in self.initialization:
            print("Using Task_residual approach for Linear Probing", end="\n")
            self.init_TR(alpha=0.5)
        elif "ClipA" in self.initialization:
            self.init_clipA()
        elif "TipA" in self.initialization:
            self.init_tipA()
        elif "CrossModal" in self.initialization:
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.adapter = LinearProbeClassifier(cfg, clip_model, base_text_features)
